module.exports = (function MakerLinkModule() {
    ...
    MLP.openTCP = function(host, port) {
        this.conn.openTCP(host, port);
        //this.clearBuffer();
        return this;
    };

    MLP.openSerial = function(port) {
        this.conn.openSerial(port);
        //this.clearBuffer();
        return this;
    };
    ...
    NetConn.prototype.openTCP = function(host,port) {
    ...
    NetConn.prototype.openSerial = function(port) {
        if (this.client) throw "already connected";
console.log({open: port});
        var conn = this;
        this.reader = new StreamReader();
        this.client = new serial(port, { baudRate: 115200 })
            .on('open', function() {
console.log({onopen: port});
                conn.writeBuffered();
                conn.emit("connect", this)
            })
...
console.log({unpack: def, buf: buf});
        var j = 0,
            off = 0,
            out = [],
            len = buf.byteLength,
